# 🤝 HANDOFF — Если ты только что открыл эту базу

Это **публичная коллаборативная база знаний** продукта **AI-Маркетолог** Влада Яско.
Любой может читать. Core-команда коммитит напрямую. Внешние партнёры — через Pull Request.

**Репо:** https://github.com/yasikvlad/ai-marketolog-knowledge-base
**Версия:** 2026-05-19 (живая, обновляется командой)
...
## 👥 Как работать в команде (одна общая база)

Эта база — **коллаборативная**. Несколько человек могут одновременно добавлять источники, заметки и claims, не мешая друг другу.

### Если ты в core-команде Влада (write-access)

1. **Клонируешь репо локально:**
   ```bash
   git clone https://github.com/yasikvlad/ai-marketolog-knowledge-base
   cd ai-marketolog-knowledge-base
   ```
2. **Открываешь в Claude Code** (или работаешь руками по METHODOLOGY.md)
3. **Добавляешь материалы** по пайплайну (Claude сам делает 10 шагов)
4. **Перед коммитом — синхронизируйся:**
   ```bash
   git pull --rebase    # подтянуть изменения коллег
   git add -A
   git commit -m "..."
   git push
   ```
5. **Если конфликт** (два человека правят один файл — обычно это INDEX.md) — Git подскажет, ручное разрешение занимает 30 сек

### Если ты внешний партнёр / маркетолог-преемник (без write-access)

1. **Форкни репо** на GitHub (кнопка Fork справа сверху)
2. Делай изменения в своём форке (тот же пайплайн через Claude Code)
3. Открой **Pull Request** в основной репо
4. Влад / core-команда ревьюит и мерджит

### Что обеспечивает качество распределённой работы

- **METHODOLOGY.md** — единые правила для всех
- **`_templates/`** — шаблоны для нового материала
- **Atomic notes** (одна идея = один файл) — специально под распределённое создание: каждый делает свои `n-*.md`, конфликтов почти нет
- **Граф `edges.jsonl`** — машиночитаемая связность, отлавливает orphan-ноты
- **Git history** — кто и когда что добавил, никто ничего не теряет

### Какие файлы конфликтуют чаще всего

- `university/INDEX.md` — добавляются строки в таблицы → решается выбором обеих строк
- `university/graph/edges.jsonl` — append-only, конфликт = «оба добавили» → склейка
- `university/queue/inbox.md` — то же самое

**Атомарные заметки и claims конфликтуют редко** — у каждого свой `n-slug-имя.md` файл.

### Запросить write-access

Напиши Владу в [@vlad_yasko_ai](https://t.me/vlad_yasko_ai) с GitHub-username — добавит коллабораторов.
